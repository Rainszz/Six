...
public class LoginUtils {
    ...
    public static boolean isLogin(){
        return PreferencesManager.getInstance(BaseApplication.getInstance()).get(Constants.IS_LOGIN,false);
    }
...
//        BaseApplication.getInstance().startActivity(new Intent(BaseApplication.getInstance(), LoginActivity.class));
    ...
    public static void autoLogin(){
        String userPhoto = PreferencesManager.getInstance(BaseApplication.getInstance()).get(Constants.USER_PHOTO);
        String userName = PreferencesManager.getInstance(BaseApplication.getInstance()).get(Constants.USER_NAME);
        String userPwd = PreferencesManager.getInstance(BaseApplication.getInstance()).get(Constants.USER_PWD);
        ...
        int loginType = PreferencesManager.getInstance(BaseApplication.getInstance()).get(Constants.LOGINTYPE,0);
        switch (loginType){
